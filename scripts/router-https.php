<?php
/* Prepare native GUI-visible router Caddy configuration, without starting it. */
...
if (!in_array($action, ['plan', 'prepare'], true) || $manifest['address'] !== '10.69.0.18') {
...
try {
    ...
    if ((string)$model->general->enabled === '1') {
        throw new RuntimeException('Preparation requires Caddy to be disabled; refusing to interrupt a live proxy');
    }
    ...
    $general = ['enabled'=>'0', 'TlsDnsProvider'=>'cloudflare',
    ...
    $wildcard = managed($model->reverseproxy->reverse, 'nixflix managed public wildcard', [
        ...
        'DnsChallenge'=>'1', 'DynDns'=>'0', 'accesslist'=>$access,
    ], 'FromDomain');
    foreach ($manifest['services'] as $name => $port) {
        ...
        $subdomain = managed($model->reverseproxy->subdomain, 'nixflix managed public name: ' . $name, [
            'enabled'=>'1', 'reverse'=>$wildcard, 'FromDomain'=>$name . '.dalebox.pw', 'DynDns'=>'0',
        ], 'FromDomain');
    ...
    foreach (['reverse', 'subdomain', 'handle'] as $group) {
        foreach ($model->reverseproxy->{$group}->iterateItems() as $uuid => $node) {
            foreach (['accesslist', 'reverse', 'subdomain'] as $field) {
    ...
    if ($action === 'prepare' && count($changes) > 0) {
        ...
        $config->save('Nixflix public HTTPS preparation; Caddy remains disabled');
    }
} finally {
    $config->unlock();
}
echo json_encode(['action'=>$action, 'changes'=>$changes, 'backup'=>$backup ?? null,
    'enabled'=>false, 'dns_token_present'=>(string)$model->general->TlsDnsApiKey !== '',
